...
void Build(const string &s) {
  for (int i = 0; i < n; i++) suf[i] = i, pos[i] = s[i] == '$' ? 0 : s[i];
...
void Num_Distinct_Substr_Lth_i() {
  int Prev = 0;
  for (int i = 0; i < n; i++)
    PREFFIX[n - suf[i] + 1]--, PREFFIX[Prev + 1]++, Prev = lcp[i];
...
void GetLCS() {
  ...
  ll LCS_lth = 0, id;
  vector<ll> f(n);
  for (int i = 0; i < n; i++) f[i] = suf[i] >= m ? 2 : 1;

  for (int i = 1; i < n; i++)
    if (f[i] != f[i - 1] && LCS_lth < lcp[i]) LCS_lth = lcp[i], id = suf[i];

  for (int i = id; i < id + LCS_lth; i++) cout << Combine[i];
}
