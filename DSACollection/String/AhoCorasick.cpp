...
struct AhoCorasick {
    ...
    for (int i = 0; i < sz(q); i++) {
      ...
      for (int c = 0; c < 26; c++)
        if (p->child[c] != NULL) {
          ...
          p->child[c] = (p == root) ? root : p->link->child[c];
    }
  }
  ...
  void Marking(const string &s, int l = 1) {
    ...
    for (char c : s) p = p->child[c - 'a'], mnz(p->fi_pos, l++);

    for (char c : s) root = root->child[c - 'a'], root->Valid = 1, root->cnt++;

    for (auto p : vector<Node *>(q.rbegin(), q.rend()))
      p->link->Valid |= p->Valid, p->link->cnt += p->cnt,
  ...
  void Print() {
    for (auto p : res)
      if (p->Valid)
        cout << "Appear " << p->cnt << " times, first at "
             << p->fi_pos - p->lth + 1 << '\n';
      else
        cout << "Not Valid\n";
  }
...
void solving() {
  ...
  for (int i = 0; i < q; i++) cin >> t, T.Add(t, i);
