...
vector<POS> snake,
    init = {POS{WTH / 2 + 1, HGT / 2, BD}, POS{WTH / 2, HGT / 2, BD},
...
bool isHitWall() {
  return !snake[0].x || !snake[0].y || snake[0].x == WTH || snake[0].y == HGT;
}
...
void startGame() {
  ...
  for (;;) {
    if (_kbhit()) {
      char ch = _getch();
      ch = tolower(ch);
      if (ch == 's' && direction != Dir::up)
        direction = Dir::down;
      else if (ch == 'w' && direction != Dir::down)
        direction = Dir::up;
      else if (ch == 'a' && direction != Dir::right)
        direction = Dir::left;
      else if (ch == 'd' && direction != Dir::left)
        direction = Dir::right;
      else if (ch == 'q') {
        ShowEndMenu();
        break;
...
void drawHeadnTail() {
  Goto(snake[0].x, snake[0].y);
  cout << snake[0].c;
  Goto(prevTail.x, prevTail.y);
  cout << ' ';
}
